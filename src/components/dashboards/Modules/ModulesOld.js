...
const Modules = () => {
        ...
        <Button
          ...
          onClick={() => {
            ...
            const sortedGroups = groupData
              ...
              .sort((a, b) => {
                return b.isChecked - a.isChecked; // Sort so that selected (checked) groups come first
              });
  ...
  const handleCheckChange = (isChecked, selectedGroup) => {
    setGroup((prevGroup) =>
      prevGroup.map((group) => {
        return group.id === selectedGroup.id ? { ...group, isChecked } : group;
      })
    );
    ...
    // const checkedGroups = updatedGroups.filter((g) => g.isChecked);
    const checkedAndFreshGroups = updatedGroups.filter(
      (g) => g.isChecked && !g.alreadyIngested
    );
  ...
  const handleUpdateGroups = async () => {
    const updatedAllowedGroups = group
      ...
      .map((group) => {
        return { ...group, ingestionCount: sliderValues[group.id] || 0 };
      });
    ...
    const invalidGroups = updatedAllowedGroups.filter((group) => {
      ...
      return !originalGroup && group.ingestionCount === 0;
    });
  ...
  const groups = [
    {
      ...
      Cell: ({ row }) => (
        <Form.Check
          ...
          checked={
            group.find((group) => group.id === row.original.id).isChecked
          }
    ...
    groups.push({
      ...
      Cell: ({ row }) => {
        ...
        const handleSliderChange = (id, newValue) => {
          const newValues = { ...sliderValues, [id]: newValue };
          ...
          if (newTotal > editingFeature.ingestion_quota_left) {
            const adjustmentRatio =
              (editingFeature.ingestion_quota_left - newValue) /
              (newTotal - newValue);
        ...
        return (
          <SliderComponent
            ...
            disabled={
              !group.find((g) => g.id === row.original.id)?.isChecked ||
              editingFeature.allowed_groups.some(
